...
public class CodeGenUtil {
	...
	public static <T> void printProcessing(Environment environment, CtType<T> ctType) {
		environment.reportProgressMessage(CLASS_TAB + "Processing class: " + ctType.getSimpleName());
	}
	...
	public static <T> boolean hasNonNullAnnotation(CtMethod<T> ctMethod) {
		return ctMethod.getAnnotations()
				.stream()
				.map(ctAnnotation -> ctAnnotation.getType()
						.getQualifiedName())
	...
	public static void addGeneratedAnnotation(CtElement ctElement, Factory factory,
		...
		CtAnnotation<Annotation> generatedAnnotation = factory.createAnnotation(
				factory.createCtTypeReference(Generated.class));
	...
	public static <T> boolean isVoidReturn(CtMethod<T> ctMethod, Factory factory) {
		return ctMethod.getType()
				.equals(factory.createCtTypeReference(void.class)) || ctMethod.getType()
				.equals(factory.createCtTypeReference(Void.class));
	}
	...
	public static <T> void addNonNullAnnotation(CtMethod<T> ctMethod, Factory factory) {
		...
		ctMethod.addAnnotation(factory.createAnnotation()
				.setAnnotationType(factory.createCtTypeReference(NonNull.class)));
	}

	public static NullityValue getNullness(CtElement ctElement, ProcessorProperties processorProperties) {
	...
	public static ModeValue getMode(CtMethod<?> ctMethod, ProcessorProperties processorProperties) {
		...
		mode = ctMethod.getParent()
				.getAnnotation(Mode.class);
		if (mode != null) {
			return mode.value();
		}
	...
	public static <T> boolean isOptimisticMode(CtMethod<T> ctMethod, ProcessorProperties processorProperties) {

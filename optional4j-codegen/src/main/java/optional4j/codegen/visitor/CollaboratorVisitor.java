...
public class CollaboratorVisitor extends CtAbstractVisitor {
	...
	public <T> void visitCtClass(CtClass<T> ctClass) {
		...
		// in order to process any NullObject or Optional return-type methods before generating other classes
		ctClass.setSimpleName(
				collaboratorAnnotation.prefix() + ctClass.getSimpleName() + collaboratorAnnotation.postfix());
		...
		CodeGenUtil.addGeneratedAnnotation(ctClass, collaboratorBuilder.getFactory(), processorClass);
		CodeGenUtil.addGeneratedAnnotation(nullObjectType, collaboratorBuilder.getFactory(), processorClass);
		CodeGenUtil.addGeneratedAnnotation(nullClass, collaboratorBuilder.getFactory(), processorClass);
	...
	public <T> void visitCtMethod(CtMethod<T> ctMethod) {
		...
		if (returnsOptionalType(ctMethod) && !returnsNullObjectType(ctMethod)) {
			ctMethod.accept(
					new ValueTypeVisitor(processorClass, environment, valueTypeBuilder, processorProperties));
			return;
		}
		...
		if (!returnsNullObjectType(ctMethod)) {
			...
			CtTypeReference<?> newReturnType = collaboratorBuilder.getNullObjectTypeReference(oldReturnType);
			changeMethodReturnTypeToNullObject(ctMethod, newReturnType);
			new NullObjectReturnReplacer(collaboratorBuilder, processorProperties).replace(ctMethod,
					newReturnType.getSimpleName(), oldReturnType);
	...
	public <T> void visitCtParameter(CtParameter<T> parameter) {
		...
		newParameter.setAssignment(collaboratorBuilder.createCodeSnippetExpression(parameter.getSimpleName()));

		method.getBody()
				.insertBegin(newParameter);
	}
	...
	private void elevateMethods(CtType<?> ctType, Set<CtMethod<?>> methods) {
		methods.stream()
				...
				.filter(method -> !Set.of("toString", "equals", "hashCode", "clone")
	...
	private <T> void addOverrideAnnotationToMethods(CtClass<T> ctClass) {
		ctClass.getMethods()
				...
				.forEach(ctMethod -> ctMethod.addAnnotation(collaboratorBuilder.createAnnotation(
						collaboratorBuilder.createCtTypeReference(Override.class))));
	}
	...
	private void implementNullInterfaceMethods(CtType<?> nullObjectType, CtClass<?> nullClass) {
		...
		nullObjectType.getMethods()
				...
				.forEach(ctMethod -> {
					...
					newMethod.setType(type);
					newMethod.addAnnotation(collaboratorBuilder.createAnnotation(
							collaboratorBuilder.createCtTypeReference(Override.class)));

					if (type.equals(collaboratorBuilder.createCtTypeReference(void.class))) {
						// do nothing
					} else if (type.equals(collaboratorBuilder.createCtTypeReference(Void.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression("null")));
					} else if (type.equals(collaboratorBuilder.createCtTypeReference(Integer.class)) || type.equals(
							collaboratorBuilder.createCtTypeReference(int.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression("0")));
					} else if (type.equals(collaboratorBuilder.createCtTypeReference(Float.class)) || type.equals(
							collaboratorBuilder.createCtTypeReference(float.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression("0.0f")));
					} else if (type.equals(collaboratorBuilder.createCtTypeReference(Double.class)) || type.equals(
							collaboratorBuilder.createCtTypeReference(double.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression("0.0d")));
					} else if (type.equals(collaboratorBuilder.createCtTypeReference(Long.class)) || type.equals(
							collaboratorBuilder.createCtTypeReference(long.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression("0L")));
					} else if (type.equals(collaboratorBuilder.createCtTypeReference(Byte.class)) || type.equals(
							collaboratorBuilder.createCtTypeReference(byte.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression("0")));
					} else if (type.equals(collaboratorBuilder.createCtTypeReference(Boolean.class)) || type.equals(
							collaboratorBuilder.createCtTypeReference(boolean.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression("false")));
					} else if (type.equals(collaboratorBuilder.createCtTypeReference(Character.class)) || type.equals(
							collaboratorBuilder.createCtTypeReference(char.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression("'\\u0000'")));
					} else if (type.equals(collaboratorBuilder.createCtTypeReference(Short.class)) || type.equals(
							collaboratorBuilder.createCtTypeReference(short.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression("0")));
					} else if (type.equals(collaboratorBuilder.createCtTypeReference(String.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression("\"\"")));
					} else if (type.isSubtypeOf(collaboratorBuilder.createCtTypeReference(Map.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression(
										"java.util.Collections.emptyMap()")));
					} else if (type.isSubtypeOf(collaboratorBuilder.createCtTypeReference(Set.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression(
										"java.util.Collections.emptySet()")));
					} else if (type.isSubtypeOf(collaboratorBuilder.createCtTypeReference(List.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression(
										"java.util.Collections.emptyList()")));
					} else if (type.isSubtypeOf(collaboratorBuilder.createCtTypeReference(Collection.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression(
										"java.util.Collections.emptyList()")));
					} else if (type.isSubtypeOf(collaboratorBuilder.createCtTypeReference(Iterable.class))) {
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression(
										"java.util.Collections.emptyList()")));
						...
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression(
										Optional.class.getSimpleName() + ".empty()")));
					} else if (type.getSimpleName()
							.startsWith(NULL_OBJ_INTERFACE_PREFIX) && type.getSimpleName()
							.endsWith(NULL_OBJ_INTERFACE_POSTFIX)) {
						// todo: use an @NullObjectReturn annotation for method returns
						block.addStatement(collaboratorBuilder.createReturn()
								.setReturnedExpression(collaboratorBuilder.createCodeSnippetExpression(
										type.getSimpleName() + "." + Collaborator.NULL_INSTANCE_FACTORY_METHOD_NAME
												+ "()")));
					} else {
						block.addStatement(collaboratorBuilder.createCodeSnippetStatement("return null"));
					}

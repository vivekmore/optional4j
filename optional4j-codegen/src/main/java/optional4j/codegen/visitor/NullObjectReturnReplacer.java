...
public class NullObjectReturnReplacer {
	...
	public void replace(CtMethod<?> ctMethod, String nullObjectTypeName, CtType<?> oldReturnType) {
		ctMethod.getBody()
				.setStatements(replace(ctMethod.getBody()
						.getStatements(), ctMethod, nullObjectTypeName, oldReturnType, 0));
	}

	private List<CtStatement> replace(List<CtStatement> originalStatements, CtMethod<?> ctMethod,
			String nullObjectTypeName, CtType<?> oldReturnType, int depth) {
		...
		for (int i = 0; i < originalStatements.size(); i++) {
			...
			if (originalStatement instanceof CtReturn<?>) {
				...
				if ("null".equals(returnStatement.getReturnedExpression()
						.toString())) {
					newStatement = collaboratorBuilder.createCodeSnippetStatement(
							nullInstanceStatement(nullObjectTypeName));
				} else {
					String localVariableName = "$nullObject" + (i + (10 * depth));
					statements.add(createLocalVariable(returnStatement, localVariableName, oldReturnType));
					if (CodeGenUtil.isOptimisticMode(ctMethod, processorProperties)) {
						newStatement = collaboratorBuilder.createCodeSnippetStatement(
								ifNullStatement(localVariableName, nullObjectTypeName));
					} else {
						newStatement = collaboratorBuilder.createCodeSnippetStatement(
								ofNullObjectStatement(localVariableName, nullObjectTypeName));
					}
				}
				...
				newBlock.setStatements(
						replace(oldBlock.getStatements(), ctMethod, nullObjectTypeName, oldReturnType, ++depth));
				newStatement = newBlock;
			} else if (originalStatement instanceof CtIf) {
				CtIf originalIf = (CtIf) originalStatement;
				CodeGenUtil.printProcessing(collaboratorBuilder.getEnvironment(), originalIf.toString());
				CtStatement thenStatement = originalIf.getThenStatement();
				CodeGenUtil.printProcessing(collaboratorBuilder.getEnvironment(), "Then: " + thenStatement.getClass());
				CtStatement replacedThen = replace(List.of(thenStatement), ctMethod, nullObjectTypeName, oldReturnType,
						++depth).get(0);
				CtIf replacedIf = collaboratorBuilder.createIf()
				...
				if (elseStatement != null) {
					CtStatement replacedElse = replace(List.of(elseStatement), ctMethod, nullObjectTypeName,
							oldReturnType, ++depth).get(0);
					CodeGenUtil.printProcessing(collaboratorBuilder.getEnvironment(),
	...
	private String ofNullObjectStatement(String localVariableName, String nullObjectTypeName) {
		return "return " + nullObjectTypeName + "." + NULL_OBJECT_FACTORY_METHOD_NAME + "(" + localVariableName + ")";
	}

	private String ifNullStatement(String localVariableName, String nullObjectTypeName) {
		return "return " + localVariableName + " != null? " + localVariableName + ": " + nullObjectTypeName + "."

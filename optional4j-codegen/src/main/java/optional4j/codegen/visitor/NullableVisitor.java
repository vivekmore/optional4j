...
public class NullableVisitor extends CtAbstractVisitor {
	...
	public <T> void visitCtMethod(CtMethod<T> ctMethod) {
		...
		if (returnsNullObjectType(ctMethod)) {
			ctMethod.accept(new CollaboratorVisitor(processorClass, environment, collaboratorBuilder, valueTypeBuilder,
					processorProperties));
			return;
		}

		if (returnsOptionalType(ctMethod) || PESSIMISTIC.equals(processorProperties.getMode())) {
			ctMethod.accept(new ValueTypeVisitor(processorClass, environment, valueTypeBuilder, processorProperties));
			return;
		}
